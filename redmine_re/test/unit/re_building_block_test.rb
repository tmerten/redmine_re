...
class ReBuildingBlockTest < ActiveSupport::TestCase
  ...
  def setup
    ...
    params = {:re_building_block => {:name => 'Note_', :artifact_type => 'ReGoal', :project_id => @project.id}}
  ...
  def test_if_right_bbs_are_delivered_for_artifact
    ...
    params_old = ReBuildingBlock.find_all_bbs_and_data(prop, @project.id)
    new_bb = ReBbText.new
    ...
    params_new = ReBuildingBlock.find_all_bbs_and_data(prop, @project.id)
