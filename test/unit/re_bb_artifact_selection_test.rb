...
class ReBbArtifactSelectionTest < ActiveSupport::TestCase
  ActiveRecord::Fixtures.create_fixtures(File.dirname(__FILE__) + '/../fixtures/', 
    [:re_building_blocks, :re_goals, :re_tasks, :re_artifact_relationships])

  def setup
    @project = Project.find(:first)
    @artifact_selection_goals = ReBbArtifactSelection.new
    params = {:re_building_block => {:name => 'Select Goals', :artifact_type => 'ReTask', :referred_artifact_types => ['ReGoal'], :referred_relationship_types => ['conflict'], :embedding_type => 'one_line', :indicate_changes => false}}
...
#  
  def test_validation_of_changed_referred_artifact
    ...
    logger.debug("______________________datum = #{datum.inspect}")
    ...
    assert @artifact_selection_goals.save
    sleep 2
    ...
    error_hash = {}
    error_hash = ReBuildingBlock.validate_building_blocks(@task_prop, error_hash, @project.id)
    logger.debug("#########################_______________ Errors on building block #{extract_error_messages(error_hash).inspect}")
