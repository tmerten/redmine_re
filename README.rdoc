...
The Redmine RE plugin (<link>[http://redmine-re.korem.de]) is a requirements engineering and requirements management plugin for the <Redmine>[http://redmine.org] project management software. The Redmine RE plugin's goal is to enable its users to create a requirements specification directly in Redmine. Users can create requirement artifacts view, store, edit and discuss these artifacts directly in Redmine.
