module Preparation
  ...
  def self.included?(base)
    # This method is called when the module is included somewhere
    # so here we can actually do the calculation of the Artifacts_types
    base.extend(Preparation::ClassMethods)
    base.artifact_types = base.create_sub_artifacts_hash
  end
  
  
  module ClassMethods
    def create_sub_artifacts_hash
      # creates a numbered hash of the symbols for my subclasses
      # which may be used for select boxes
      symbols = {:ReGoal => 0, :ReTask => 1, :ReSubtask => 2, :ReWorkarea => 3, :ReUserProfile => 4, :ReSection => 5}
#      i = 0
#      ObjectSpace.each_object() do |c|
#        next unless c.include?(:Artifact)
#        symbols[c.to_s.to_sym] = i
#        i += 1
#      end
      symbols
    end
